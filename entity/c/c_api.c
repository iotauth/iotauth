...
//client만 이용함.
//config에 필요한거: IP_ADDRESS PORT_NUM sender purpose num_key
...
unsigned char * auth_hello_reply_message(unsigned char * entity_nonce, unsigned char * auth_nonce, unsigned char num_key, unsigned char * sender, unsigned int sender_length, unsigned char* purpose, unsigned int purpose_length, unsigned int * ret_length)
{
    ...
    memcpy(ret + NONCE_SIZE*2 + NUMKEY_SIZE + 1, sender, sender_length);
    memcpy(ret + NONCE_SIZE*2 + NUMKEY_SIZE + 1 + sender_length, temp2, 1);
    memcpy(ret + NONCE_SIZE*2 + NUMKEY_SIZE + 1 + sender_length +1, purpose, purpose_length);
    *ret_length = NONCE_SIZE*2 + NUMKEY_SIZE + 1 + sender_length +1 + purpose_length;
}
