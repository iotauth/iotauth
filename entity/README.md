...
The Python API (`python/`) uses [Ruff](https://docs.astral.sh/ruff/) for linting and formatting.
...
Ruff is configured in `python/pyproject.toml`. All contributions to the Python API should pass `ruff check .` with no errors before submitting a pull request.
