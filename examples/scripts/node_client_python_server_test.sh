...
EOF
	...
	tmux send-keys -t "$SERVER_PANE" "sleep 3 && cd $(quote_for_shell "$SST_ROOT/entity/python/examples") && source ../.venv/bin/activate && while true; do python3 -u py_server.py configs/py_server.config; done" C-m
...
start_service server bash -c \
	"cd $(quote_for_shell "$SST_ROOT/entity/python/examples") && source ../.venv/bin/activate && while true; do python3 -u py_server.py ../../node/example_entities/configs/net1/server.config; sleep 0.5; done"
