 ...
 *
 */
public class LocalConnector extends org.eclipse.jetty.server.AbstractConnector
{
    private final BlockingQueue<LocalEndPoint> _connects = new LinkedBlockingQueue<>();

    public LocalConnector(Server server, Executor executor, Scheduler scheduler, ByteBufferPool pool, int acceptors, org.eclipse.jetty.server.ConnectionFactory... factories)
    {
    ...
    {
        this(server, null, null, null, -1, new org.eclipse.jetty.server.HttpConnectionFactory());
    }

    public LocalConnector(Server server, SslContextFactory sslContextFactory)
    {
        this(server, null, null, null, -1, org.eclipse.jetty.server.AbstractConnectionFactory.getFactories(sslContextFactory,new HttpConnectionFactory()));
    }

    public LocalConnector(Server server, org.eclipse.jetty.server.ConnectionFactory connectionFactory)
    {
