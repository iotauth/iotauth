...
{       
    ...
    throws IOException
    {
        try
        {
            ...
            if (request instanceof org.eclipse.jetty.server.Request)
            {
                ...
                HttpFields fields = ((org.eclipse.jetty.server.Request)request).getHttpFields();
    ...
    throws IOException
    {
        ...
        try
        {
            ...
            written = out instanceof org.eclipse.jetty.server.HttpOutput
                ? ((org.eclipse.jetty.server.HttpOutput)out).isWritten()
                : true;
        }
        ...
        if ( reqRanges == null || !reqRanges.hasMoreElements() || content_length<0)
        {
            ...
            else if (written || !(out instanceof org.eclipse.jetty.server.HttpOutput))
            {
                // write normally
            ...
            else
            {
                ...
                if (request.isAsyncSupported() && content.getContentLengthValue()>response.getBufferSize())
                {
                    ...
                    ((org.eclipse.jetty.server.HttpOutput)out).sendContent(content,new Callback()
                    {
                        @Override
        ...
        else
        {
            ...
            List<org.eclipse.jetty.server.InclusiveByteRange> ranges = org.eclipse.jetty.server.InclusiveByteRange.satisfiableRanges(reqRanges,content_length);
            ...
            if (ranges==null || ranges.size()==0)
            {
                ...
                response.setHeader(HttpHeader.CONTENT_RANGE.asString(),
                        org.eclipse.jetty.server.InclusiveByteRange.to416HeaderRangeString(content_length));
                content.getResource().writeTo(out,0,content_length);
                return true;
            }
            ...
            if ( ranges.size()== 1)
            {
                org.eclipse.jetty.server.InclusiveByteRange singleSatisfiableRange = ranges.get(0);
            ...
            {
                org.eclipse.jetty.server.InclusiveByteRange ibr = ranges.get(i);
