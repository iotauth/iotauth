...
/**
 * {@link HttpInput} provides an implementation of {@link ServletInputStream} for {@link org.eclipse.jetty.server.HttpChannel}.
 * <p>
...
public class HttpInput extends ServletInputStream implements Runnable
{
    ...
    public HttpInput(org.eclipse.jetty.server.HttpChannelState state)
    {
        _channelState = state;
    }
    ...
    public void run()
    {
        ...
        try
        {
            if (error != null)
            {
                ...
                _channelState.getHttpChannel().getResponse().getHttpFields().add(org.eclipse.jetty.server.HttpConnection.CONNECTION_CLOSE);
