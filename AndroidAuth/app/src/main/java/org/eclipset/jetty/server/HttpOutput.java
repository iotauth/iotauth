...
public class HttpOutput extends ServletOutputStream implements Runnable
{
     ...
     * <p>
     * The {@link org.eclipse.jetty.server.HttpChannel} is an {@link Interceptor} and is always the
    ...
    */
    ...
    public HttpOutput(org.eclipse.jetty.server.HttpChannel channel)
    {
        _channel = channel;
        _interceptor = channel;
        _writeBlocker = new WriteBlocker(channel);
        org.eclipse.jetty.server.HttpConfiguration config = channel.getHttpConfiguration();
    ...
    public void recycle()
    {
        ...
        org.eclipse.jetty.server.HttpConfiguration config = _channel.getHttpConfiguration();
        _bufferSize = config.getOutputBufferSize();
        _commitSize = config.getOutputAggregationSize();
        if (_commitSize > _bufferSize)
            _commitSize = _bufferSize;
     ...
     * InputStream and write it to the associated {@link org.eclipse.jetty.server.HttpChannel}.
     * A non direct buffer of size {@link HttpOutput#getBufferSize()} is used.
     * This callback is passed to the {@link org.eclipse.jetty.server.HttpChannel#write(ByteBuffer, boolean, Callback)} to
     ...
     * ReadableByteChannel and write it to the {@link org.eclipse.jetty.server.HttpChannel}.
     * A {@link ByteBuffer} of size {@link HttpOutput#getBufferSize()} is used that will be direct if
     * {@link org.eclipse.jetty.server.HttpChannel#useDirectBuffers()} is true.
     * This callback is passed to the {@link org.eclipse.jetty.server.HttpChannel#write(ByteBuffer, boolean, Callback)} to
     * be notified as each buffer is written and only once all the input is consumed will the
     * wrapped {@link Callback#succeeded()} method be called.
     */
